...
public class Web_Scrap_PartA {
	...
	public static Sheet sheet1 = wb.createSheet("Other tags Sheet");
	...
	public static void main(String[] args) {
		try 
		{
			Web_Scrap_PartA obj=new Web_Scrap_PartA();
			Row row1 =sheet1.createRow(0);
			row1.createCell(0).setCellValue("Tags");
			row1.createCell(1).setCellValue("Text");
			Row row2 =sheet2.createRow(0);
			row2.createCell(0).setCellValue("Text");
			row2.createCell(1).setCellValue("Links");
...
//				if(TrUrl.getKey().isEmpty() || TrUrl.getKey().contains(".pdf") || TrUrl.getKey().contains(".PDF") || TrUrl.getKey().endsWith(".doc") || TrUrl.getKey().contains(".docx") || TrUrl.getKey().endsWith(".xls") || TrUrl.getKey().contains(".xlsx") || TrUrl.getKey().endsWith(".jpg") || TrUrl.getKey().endsWith(".JPG") || TrUrl.getKey().endsWith(".jpeg") || TrUrl.getKey().endsWith("#") || TrUrl.getKey().endsWith("#main-content") || TrUrl.getKey().endsWith("#0") || TrUrl.getKey().startsWith("mailto"))
//					continue;
				obj.getUrls(TrUrl.getKey(), TrUrl.getValue());
			}
			try (OutputStream fileOut = new FileOutputStream("PartA.xls")) {
			    wb.write(fileOut);
			}
			wb.close();
